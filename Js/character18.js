window.addEventListener("load", function () {
    ...
    fetch('https://breakingbadapi.com/api/characters')
        ...
        .then(character1 => {
            ...
            h2.innerHTML = `Nombre: ${character1[17].name}`
            divTitle.appendChild(h2)
            //Photo
            let img = document.createElement('img');
            img.src = character1[17].img;
            img.width = "300"
            img.height = "300"
            divImg.appendChild(img);
            //Information
            let p = document.createElement('p')
            p.innerHTML = `<b>ID</b>: ${character1[17].char_id} <br/> <b>Nacimiento</b>: ${character1[17].birthday} <br/> <b>Apodo</b>: ${character1[17].nickname}
                <br/> <b>Temporadas</b>: ${character1[17].appearance} <br/> <b>Actor</b>: ${character1[17].portrayed}`
            divId.appendChild(p)
            span();
        })
